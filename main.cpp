...
template<class T>
int Cola<T>::busquedaLineal(T& elem){
    for (int i = 0; i <= ult; i++){
        if (elem == datos[i]){
            return ult - i;
        }
...
int main()
{
    ...
    do
    {
        ...
        std::cout<<"3. Buscar una constancia"<<std::endl;
        ...
        switch (opc)
        {
            ...
            case 3:
                system("cls");
                std::cout<<"Buscar una constancia\n"<<std::endl;
                std::cout<<"Ingrese los datos de la constancia a buscar"<<std::endl;
                std::cin>>x;
                std::cout<<"\nFaltan "<<MiCola.busquedaLineal(x)<<" Constancias para llegar a la buscada"<<std::endl;
                system("pause");
        }
    } while (opc != 4);
